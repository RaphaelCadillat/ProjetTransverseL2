...
<html>
    ...
    <body>
        ...
        try{
            ...
            $req_list_friends = $PDO->prepare('SELECT * FROM req_friends WHERE (id_req_from=? OR id_req_to=?) AND req_statuts=?');
            $req_list_friends->bindValue(1, $id_user);
            $req_list_friends->bindValue(2, $id_user);
            $req_list_friends->bindValue(3, 0);
            $req_list_friends->execute();
            $list_friends = $req_list_friends->fetchAll();
        ...
        for($i=0;$i<sizeof($data_allevents);$i++){
            ...
            for($j=0;$j<sizeof($list_friends);$j++)
            {
                
                if((($data_allevents[$i]["id_user"]==$list_friends[$j]["id_req_from"] && $id_user==$list_friends[$j]["id_req_to"]) || ($data_allevents[$i]["id_user"]==$list_friends[$j]["id_req_to"]&&$id_user==$list_friends[$j]["id_req_from"])) && $list_friends[$j]["req_statuts"]==0)// s il sont amis
                {
                    echo '<div>';
                    echo '<p>Event Name :</p>';
